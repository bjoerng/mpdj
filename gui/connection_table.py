...
from PyQt6.QtWidgets import QTableWidget, QComboBox, QMenu, QLabel, QMessageBox
...
from quodlibet.qltk.songlistcolumns import _highlight_current_cell
...
class ConnectionTableWidget(QTableWidget):
    ...
    def initiate_combo_boxes(self):
        ...
        block_signals_tmp = self.blockSignals(True)
        ...
        for column in range(0,column_count - 1):
            for row in range(0,row_count - 1):
                ...
                current_cell_widget = self.cellWidget(row, column)
                if not isinstance(current_cell_widget, QComboBox):
                    current_cell_widget = QComboBox()
                    current_cell_widget.addItems(['0','1'])
                    current_cell_widget.setProperty('row', row)
                    current_cell_widget.setProperty('column',column)
                    current_cell_widget.currentIndexChanged.connect(
                        self.artist_connection_combo_box_changed_indexchanged)
                    self.setCellWidget(row, column,  current_cell_widget)                 
                is_connected_str = str(is_connected)
                item_index = current_cell_widget.findText(is_connected_str)
                if item_index != -1:
                    cell_widget_signals_blocked_tmp = current_cell_widget.blockSignals(True)
                    current_cell_widget.setCurrentIndex(item_index)
                    current_cell_widget.blockSignals(cell_widget_signals_blocked_tmp)
        self.blockSignals(block_signals_tmp)
    ...
    def show_header_right_click_menu(self, position):
        ...
        menu = QMenu()
        logical_index_x = self.horizontal_header.logicalIndexAt(position.x())
        logical_index_y = self.vertical_header.logicalIndexAt(position.y())
    ...
    def __init__(self):
        ...
        self.horizontal_header.customContextMenuRequested.connect(self.show_header_right_click_menu)
        self.vertical_header = self.verticalHeader()
        self.vertical_header.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        self.vertical_header.customContextMenuRequested.connect(self.show_header_right_click_menu)
